...
* Define a Verified Telemetry User Initialization wrapper function which will make calls to necessary initilization function from the vT library
	...
	void * sample_pnp_verified_telemetry_user_init()
	{
	    ...
	    {
		VT_DEBUG_PRINT(USER,"Failed to initialize vTTelemetry1 component: error code = 0x%08x\r\n", status);
	    }
		...
		// 					    NUMBER_OF_VT_ENABLED_TELEMETRIES)))
    	// {
    	//     VT_DEBUG_PRINT(USER,"Failed to initialize vTTelemetry2 component: error code = 0x%08x\r\n", status);
    	// }
						    ...
						    NUMBER_OF_VT_ENABLED_TELEMETRIES )))
	    {
		VT_DEBUG_PRINT(USER,"Failed to initialize vTTelemetryN component: error code = 0x%08x\r\n", status);
	    }
	    else if ((status = pnp_vt_init(&verified_telemetry_DB, fallcurve_components, NUMBER_OF_VT_ENABLED_TELEMETRIES, 
							true, 0x080E0000 )))
	    {
		VT_DEBUG_PRINT(USER,"Failed to initialize Verified Telemetry: error code = 0x%08x\r\n", status);
	    }
