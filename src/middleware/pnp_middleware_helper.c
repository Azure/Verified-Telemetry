...
static bool is_component_in_model(
    az_iot_pnp_client const* client, az_json_token const* component_name, az_span* out_component_name)
{
    int32_t index = 0;
...
static az_result check_if_skippable_reported(az_json_reader* jr, az_iot_pnp_client_property_response_type response_type)
{
    az_result _az_result;
    // First time move
    ...
    while (true)
    {
        ...
        else if ((response_type == AZ_IOT_PNP_CLIENT_PROPERTY_RESPONSE_TYPE_DESIRED_PROPERTIES &&
                     jr->_internal.bit_stack._internal.current_depth == 2) ||
                 (response_type == AZ_IOT_PNP_CLIENT_PROPERTY_RESPONSE_TYPE_GET &&
    ...
    UINT message_properties_buffer_size)
{
    ...
    if ((pnp_client_ptr == NX_NULL) || (packet_pptr == NX_NULL))
    {
        LogError(LogLiteralArgs("IoT PnP telemetry message create fail: INVALID POINTER"));
        return (NX_AZURE_IOT_INVALID_PARAMETER);
    }

    status = nx_azure_iot_hub_transport_publish_packet_get(
        &(pnp_client_ptr->nx_azure_iot_pnp_client_transport), &packet_ptr, wait_option);
    if (status)
    {
