...
static VT_VOID remove_harmonics(SPECTOGRAM *spectogram_object, VT_INT start_index, VT_INT samples, VT_FLOAT sampling_frequency)
{
  ...
  for(VT_INT iter1 = start_index; iter1 < (samples + start_index - 1); iter1++)
  {
    ...
    for(VT_INT iter2 = iter1+1; iter2 < (samples + start_index); iter2++)
    {
      ...
      frequency_allowed_delta = ((sampling_frequency / (VT_FLOAT) VT_CS_SAMPLE_LENGTH) * ((VT_FLOAT)round(frequency_multiplier))) / spectogram_object[iter1].frequency;
...
static VT_VOID calculate_top_N_signal_frequencies(SPECTOGRAM *spectogram_object, VT_INT start_index, COMPLEX* signal, VT_FLOAT sampling_frequency)
{
  #if VT_LOG_LEVEL > 2
    VT_INT decimal;
    VT_FLOAT frac_float;
  ...
  for(VT_INT iter = 0; iter < VT_CS_MAX_TEST_FREQUENCIES; iter++)
  {
    ...
    {
      average_of_peak_neighbour = (signal[peak_index - 1].real + signal[peak_index].real + signal[peak_index + 1].real)/3.0f;
      signal[peak_index - 1].real = average_of_peak_neighbour;
      signal[peak_index].real = average_of_peak_neighbour;
      signal[peak_index + 1].real = average_of_peak_neighbour;
    }
...
VT_VOID cs_signature_compute_collection_settings(VT_CURRENTSENSE_OBJECT* cs_object, VT_FLOAT *top_N_sample_frequencies, VT_FLOAT* lowest_sample_freq)
{
  ...
  for (VT_INT iter = 0; iter < calib_ranges; iter++)
  {
    ...
    cs_adc_read(cs_object, adc_read_signal, get_calib_range_freq(iter), VT_CS_SAMPLE_LENGTH);
    //Add Digital Filter
