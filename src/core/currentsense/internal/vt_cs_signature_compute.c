...
static VT_UINT8 check_acr_peak_present(VT_FLOAT *raw_signature, VT_UINT *index, VT_INT period, VT_FLOAT *period_total, VT_UINT *peaks)
{
...
VT_UINT cs_signature_compute(VT_CURRENTSENSE_OBJECT* cs_object, VT_FLOAT sampling_frequency, VT_FLOAT *signature_frequency, VT_FLOAT *duty_cycle, VT_FLOAT *current_draw)
{
  ...
  cs_adc_read(cs_object, raw_signature, sampling_frequency, VT_CS_SAMPLE_LENGTH);
...
VT_VOID cs_signature_avg_compute(VT_CURRENTSENSE_OBJECT* cs_object, VT_FLOAT* avg_curr)
{
  ...
  VT_FLOAT avg_curr_compute = 0;
    ...
    VT_INT frac; 
  #endif /* VT_LOG_LEVEL > 2 */
  cs_adc_read(cs_object, adc_read_signal, cs_object->fingerprintdb.lowest_sample_freq, VT_CS_SAMPLE_LENGTH);
