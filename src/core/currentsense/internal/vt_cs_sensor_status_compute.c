...
VT_VOID cs_sensor_status(VT_CURRENTSENSE_OBJECT* cs_object)
{
    if (cs_object->fingerprintdb.template_type == VT_CS_NON_REPEATING_SIGNATURE)
    {
        ...
        if (cs_fetch_non_repeating_signature_average_current(cs_object, &avg_curr_on_saved, &avg_curr_off_saved) == VT_SUCCESS)
        {
            if (cs_non_repeating_signature_average_current_compute(cs_object, &avg_curr_on, &avg_curr_off) == VT_SUCCESS)
            {
    ...
    else
    {
        ...
        if (cs_fetch_repeating_signature_offset_current(cs_object, &lowest_sample_freq_saved, &offset_current_saved) ==
            VT_SUCCESS)
        {
        ...
        {
            if (cs_repeating_signature_offset_current_compute(cs_object, lowest_sample_freq_saved, &offset_current) == VT_SUCCESS)
            {
        ...
        {
            if (cs_fetch_repeating_signature_feature_vector(cs_object,
                    iter,
            ...
            if (cs_repeating_signature_feature_vector_compute(
                    cs_object, sampling_frequency_saved, &signature_frequency, &duty_cycle, &relative_current_draw))
            {
