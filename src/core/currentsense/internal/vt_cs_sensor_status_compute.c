...
VT_VOID cs_sensor_status(VT_CURRENTSENSE_OBJECT* cs_object)
{
    ...
    {
        cs_signature_features_avg_compute(cs_object, lowest_sample_freq_saved, &avg_curr);
        avg_curr_drift += cs_signature_features_avg_evaluate(avg_curr, avg_curr_saved);
    }

    for (VT_UINT iter = 0; iter < VT_CS_MAX_SIGNATURES; iter++)
    {
        if (cs_fetch_signature(
                cs_object, iter, &sampling_frequency_saved, &signature_frequency_saved, &duty_cycle_saved, &current_draw_saved))
        {
            break;
        }
        if (cs_signature_features_compute(cs_object, sampling_frequency_saved, &signature_frequency, &duty_cycle, &current_draw))
        {
            signature_compute_fail = true;
            break;
        }
        signature_drift += cs_signature_features_evaluate(
            signature_frequency, signature_frequency_saved, duty_cycle, duty_cycle_saved, current_draw, current_draw_saved);
    ...
    else if ((signatures_evaluated == 0) && avg_current_unavailable)
    {
    ...
    else if ((avg_curr_drift > VT_CS_MAX_AVG_CURR_DRIFT) || (signature_drift > VT_CS_MAX_SIGNATURE_DRIFT))
    {
        ...
        {
            cs_object->sensor_drift = (avg_curr_drift + signature_drift) / 2;
        }
        else if (signatures_evaluated)
        {
            cs_object->sensor_drift = signature_drift;
        }
        else
        {
