    ...
    VT_FLOAT current_draw_saved)
{
    ...
    signature_drift += fabsf((current_draw_under_test - current_draw_saved) / current_draw_saved) * 100.0f;
...
{
    VT_FLOAT signature_drift = 0;
    signature_drift          = fabsf((avg_curr_under_test - avg_curr_saved) / avg_curr_saved) * 100.0f;
#if VT_LOG_LEVEL > 2
    int32_t decimal  = signature_drift;
    float frac_float = signature_drift - (float)decimal;
    int32_t frac     = frac_float * 10000;
