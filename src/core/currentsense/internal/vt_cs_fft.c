...
VT_VOID cs_fft_windowing(COMPLEX *Y, VT_UINT N, VT_UINT8 windowType, VT_UINT8 dir)
{
	...
	for (VT_UINT i = 0; i < (N >> 1); i++) 
   {
		...
		case FFT_WIN_TYP_NUTTALL: // nuttall
			weighingFactor = 0.355768f - (0.487396f * ((VT_FLOAT)cos(twoPi * ratio))) + (0.144232f * ((VT_FLOAT)cos(fourPi * ratio))) - (0.012604f * ((VT_FLOAT)cos(sixPi * ratio)));
			break;
		case FFT_WIN_TYP_BLACKMAN: // blackman
			weighingFactor = 0.42323f - (0.49755f * ((VT_FLOAT)cos(twoPi * ratio))) + (0.07922f * ((VT_FLOAT)cos(fourPi * ratio)));
			break;
		case FFT_WIN_TYP_BLACKMAN_NUTTALL: // blackman nuttall
			weighingFactor = 0.3635819f - (0.4891775f * ((VT_FLOAT)cos(twoPi * ratio))) + (0.1365995f * ((VT_FLOAT)cos(fourPi * ratio))) - (0.0106411f * ((VT_FLOAT)cos(sixPi * ratio)));
			break;
		case FFT_WIN_TYP_BLACKMAN_HARRIS: // blackman harris
			weighingFactor = 0.35875f - (0.48829f * ((VT_FLOAT)cos(twoPi * ratio))) + (0.14128f * ((VT_FLOAT)cos(fourPi * ratio))) - (0.01168f * ((VT_FLOAT)cos(sixPi * ratio)));
			break;
		case FFT_WIN_TYP_FLT_TOP: // flat top
			weighingFactor = 0.2810639f - (0.5208972f * (VT_FLOAT)cos(twoPi * ratio)) + (0.1980399f * (VT_FLOAT)cos(fourPi * ratio));
			break;
...
VT_VOID cs_fft_major_peak(COMPLEX *Y, VT_UINT N, VT_FLOAT sampling_freq, VT_FLOAT *f, VT_FLOAT *v, VT_INT *index)
{
	...
	VT_FLOAT delta = 0.5f * ((Y[IndexOfMaxY - 1].real - Y[IndexOfMaxY + 1].real) / (Y[IndexOfMaxY - 1].real - (2.0f * Y[IndexOfMaxY].real) + Y[IndexOfMaxY + 1].real));
